  ...
  if (variant) {
    ...
    if (variant.available) {
      $addToCart.removeClass('disabled').prop('disabled', false);
        ...
        $addToCartText.html('REALIZAR MI PEDIDO');
      
       
      $AddToCartTextfree.html('COMPRA AHORA CON ENVÍO GRATIS');
      ...
      if (variant.compare_at_price > variant.price) {
        $comparePrice.html("<span class='money'>" + Shopify.formatMoney(variant.compare_at_price, moneyFormat) + "</span>").show();
         $comparePrice2.html("<span class='money'>" + Shopify.formatMoney(variant.compare_at_price, moneyFormat) + "</span>").show();
          ...
          $youSave.html('Descuento <b id="save_value-'+product_id+'"><span class="money">'+Shopify.formatMoney((variant.compare_at_price - variant.price), moneyFormat)+'</span> ('+ per.toFixed(0) +'%)</b>').show();
      ...
      $productPrice.html("No disponible");
      $productPrice2.html("No disponible");
      $comparePrice.hide();
        $comparePrice2.hide();
      $comparePriceClass.hide();
...
timber.bxSliderApply = function (options) {
  ...
  if(options.total_images > 1) {
    ...
    if(options.total_images > 1) { // was 2; changed to 1 on 2021-04-08
    ...
    jQuery.each(cart, function(key, value) {
      if (key === 'items') {
        ...
        if (value.length) {
          ...
          $html += '<li><a href="/cart" class="btn--secondary btn--full cart__cartview"><span class="fas fa-shopping-cart button-cart-icon"></span> VER MI CARRITO</a></li><li>';
          ...
          $html += '<span id="CheckoutText">CONTINUAR MI PEDIDO</span>';
          $html += '</a>';
          ...
          $html +=  '</li><img alt="Checkout Secure" src="//sotoimports.com/cdn/shop/t/4/assets/checkout_icon.png?v=56498116546714630641711868697" class="no-border checkout-img ratina-img">';
          ...
          $html = '<div class="container"><ul class="cart-popup-ul-bottom"><li class="empty-cart-popup-msg">Su carrito actualmente está vacío.</li></ul><a href="/checkout" class="btn btn--full cart__checkout Checkout checkout-dummy-button"></a></div>';
      ...
      if (key === 'items') {
        ...
        if (value.length) {
          jQuery.each(value, function(i, item) {
            ...
            $html += '<span class="visually-hidden">Quitar uno a la cantidad de artículos</span>';
            $html += '</button>';
            ...
            $html += '<span class="visually-hidden">Aumentar uno a la cantidad de artículos</span>';
            $html += '</button>';
            $html += '</div></div>';
            $html += '</td>';
...
weekday[6] = "sabado";
    var n = weekday[d.getDay()];
     $('#DCTime').html(n);
      ...
      var supported_currencies = 'USD EUR JPY CAD INR GBP AUD PEN';
  ...
  if(flag_addcart) {
    ...
    params = {
      ...
      success: function(line_item) {
          ...
          console.log('Skipping cart, checking out in 100 ms :)');
          setTimeout(function(){
...
var ajaxCart = (function(module, $) {
  ...
  formOverride = function () {
    $formContainer.on('submit', function(evt) {
      ...
      $addToCart.removeClass('is-added').addClass('is-adding');
        ...
        $addToCart.find('span#AddToCartText').html('Agregando al Carrito');
  ...
  itemAddedCallback = function (product) {
    ...
    setTimeout(function () {
      $addToCart.removeClass('is-adding').addClass('is-added');
        ...
        $addToCart.find('span#AddToCartText').html('Producto Agregado');
    ...
    setTimeout(function () {
      $addToCart.removeClass('is-adding is-added');
        ...
        $addToCart.find('span#AddToCartText').html('REALIZAR MI PEDIDO');
  ...
  buildCart = function (cart) {
    ...
    // Show empty cart
    if (cart.item_count === 0) {
      $cartContainer.append('<p>' + "Su carrito actualmente está vacío." + '</p>');
      cartCallback(cart);
    ...
    if (numInputs.length) {
      ...
      $('.js-qty__adjust').on('click', function() {
        var $el = $(this),
            id = $el.data('id'),
            //$qtySelector = $el.siblings('.js-qty__num'),
            $qtySelector = $('.js-qty__num'), // target both of them
            qty = parseInt($qtySelector.val().replace(/\D/g, ''));
      ...
      $.each($('.r_addtocartbutton button'), function(k, v) {
        ...
        $.each(product_ids_in_cart, function (x, y) {
          if ($(v).attr('onclick').indexOf(y) != -1) {
            btn.attr('disabled', 'disabled' ).find('.r_AddToCartText').text('In cart').css('color', '#1dbf73');
                                                                                           }
                                                                                           });
          });
    ...
    if($(this).find('.panel').hasClass('active')){
      ...
      function free_shipping_update() {
        ...
        var badage_bar_msg = $('#badage_shipping_msg').html();
            var badage_msg_free = $('#badage_free_shipping_msg').html();
        ...
        if(badage_bar_msg != "" && badage_bar_msg != undefined){
            $(".badage_shipping_text").html(common_bar_msg.replace('XX', formated).replace('$$', formated).replace('$x', formated).replace('$X', formated));
          }
